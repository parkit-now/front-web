...
/**
 * Isotipo oficial de Mercado Pago: el apretón de manos dentro del óvalo.
 *
 * El path es el auténtico de `simple-icons`
 * (https://cdn.jsdelivr.net/npm/simple-icons/icons/mercadopago.svg), que ya
 * viene en un viewBox 24x24 igual que el resto del barrel: entra sin
 * estirarse ni recortarse.
 *
 * Ojo con dos cosas que lo separan de sus 52 hermanos:
 *
 * 1. Es de relleno, no de trazo. Por eso pisa el `fill`/`stroke` que pone
 *    `Icon` por defecto. `strokeWidth` queda de adorno con `stroke="none"`.
 * 2. Es una marca, y las marcas tienen color propio. Aun así hereda
 *    `currentColor` como todos: es la única forma de que en el sidebar quede
 *    monocromo junto a los otros 10 iconos y siga el estado activo. Donde hay
 *    que mostrar la marca —la tarjeta de Integraciones— el color se inyecta
 *    desde afuera con `var(--mp-brand)`. Un celeste hardcodeado acá rompería
 *    el sidebar.
 */
export const IconMercadoPago = (p: IconProps) => (
